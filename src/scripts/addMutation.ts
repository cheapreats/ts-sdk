...
`
    ...
    if (appContent.length !== 2) {
      throw new Error(
        "App file must have exactly one block of // ADD CONTROLLER IMPORTS BELOW"
      );
    }
...
const ${name}Controller = require('./controllers/${name}Controller');`;
    ...
    if (appContent.length !== 2) {
      throw new Error(
        "App file must have exactly one block of // ADD CONTROLLERS BELOW"
      );
    }
    ...
    if (appContent.length !== 2) {
      throw new Error(
        "App file must have exactly one block of // ADD GETTERS BELOW"
      );
    }
...
(async () => {
  const controllerFilePath = `./src/app/controllers/${args[0]}Controller`;
  const appFilePath = `./src/app/App`;
  ...
  if (content.length !== 2) {
    throw new Error(
      "Controller file must have exactly one block of // ADD MUTATION METHODS BELOW"
    );
  }
  ...
  if (content.length !== 2) {
    throw new Error(
      "Controller file must have exactly one block of // ADD BINDINGS BELOW"
    );
  }
